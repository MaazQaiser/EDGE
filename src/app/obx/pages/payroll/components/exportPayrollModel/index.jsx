...
import { Alert, Box, Checkbox, InputLabel } from '@mui/material';
...
const fileNamePrefixes = {
  lockedPayrun: 'locked_payrun',
  payroll: 'approved_payroll',
};

const ExportPayrollModel = ({ open, onClose, isLocked }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const startDate = windowStartDateWrtFranchise;
      const endDate = windowEndDateWrtFranchise;
      const payload = {
      ...
      if (isLocked) payload.isLocked = true;
  ...
  const getPayrollFilename = () => {
    const prefix = isLocked ? fileNamePrefixes.lockedPayrun : fileNamePrefixes.payroll;
    const timestamp = dayjs().format('MM_DD_YYYY_hh_mm_a');
    return `${prefix}_${timestamp}.csv`;
  };

  const addSelectedHoursBody = (
    <Box className={classes.rejectModal}>
      <CSVLink filename={getPayrollFilename()} ref={csvRef} data={csvPayload || []}></CSVLink>
      ...
      <Box className={classes.rejectModalInner}>
        ...
        <Alert icon={<Info />} severity="info" className={classes.grayAlert}>
          {t('obx.payroll.exportPayrollDescription')}
        </Alert>
        ...
        <Box className={classes.internalMapBox}>
          ...
          />
          <InputLabel>{t('obx.payroll.splitShiftsInTwoAtMidnight')}</InputLabel>
        </Box>
